...
자동화가 생성한 분석을 단순 요약으로 축소하지 않도록 아래 상세 필드도 함께 보존합니다.

- `sourceWindow` — 지표별 실제 확인 시간 범위와 지연 가능성
- `rates` — 명목금리·실질금리·수익률곡선·기대인플레이션 구조
- `detailSections` — 금리·달러, 귀금속, 에너지, 주가지수별 수치·해석·상승·하락 조건
- `eventFocus` — 당일 핵심 발표의 예상·이전치·세부 확인 항목·결과별 시장 경로
- `marketClose`와 `equity` — 주요 지수 종가와 종목·업종·실적 해석
...
- `watchPoints` — 발표 시 함께 확인할 세부 항목
- `marketImpact` — 결과가 금리·환율·주식·원자재로 전달되는 경로
